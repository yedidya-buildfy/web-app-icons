...
-- Optional: a view for simpler selection and future-proofing
CREATE OR REPLACE VIEW public.generated_icons_view AS
SELECT id, deterministic_id, icon_name, subject, style, colors, background, image_url, created_at
FROM public.generated_icons;

-- RLS policies (public read, insert allowed with anon but only valid columns)
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Allow read to all" ON public.generated_icons
      FOR SELECT
      USING (true);
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Allow insert to anon" ON public.generated_icons
      FOR INSERT
      WITH CHECK (true);
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Allow update on conflict" ON public.generated_icons
      FOR UPDATE
      USING (true)
      WITH CHECK (true);
  END IF;
